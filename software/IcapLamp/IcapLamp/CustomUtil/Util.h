#ifndef __CUTIL_H__
#define __CUTIL_H__

#include <string>
#include <sstream>
#include <vector>

using namespace std;

//常用函数封装
namespace CUtil
{
	//CRC表
	const unsigned char CDT_BCH[] =
	{
		
	};

	//用于url字符转义
	inline unsigned char toHex(const unsigned char &x)
	{
		return x > 9 ? x -10 + 'A': x + '0';
	}

	inline unsigned char fromHex(const unsigned char &x)
	{
		return isdigit(x) ? x-'0' : x-'A'+10;
	}

	//辅助函数，解决中文路径的输出
	std::string EncodeUtf8(std::wstring in);
	std::wstring DecodeUtf8(std::string in);

	//url转义
	std::string URLEncode(const std::string &sIn);
	std::string URLDecode(const std::string &sIn);

	//unicode to ascii
	void GetAnsiString(const CString &s, char dest[], size_t total);
	void GetGB2312String(CString &strInUnicodeStr, char strOutGb2312Str[], size_t size);

	//获取当前程序运行路径
	void GetCurrentExecPath(char * fileName,char* filePath);

	//去除字符串左右空白
	std::string trim(const std::string& str);

	//根据指定字符分割
	int split(const std::string& str, std::vector<std::string>& ret_, std::string sep = ",");

	//用指定字符替换
	std::string replace(const std::string& str, const std::string& src, const std::string& dest);

	//计算CRC值
	unsigned char CRCCacu(const unsigned char *ptr,  unsigned int size);
};
#endif	